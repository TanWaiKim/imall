...
import com.imall.vo.UserListVo;
...
public class UserServiceImpl implements IUserService {
     ...
     * @return
     */
    public ServerResponse<UserDetailVo> manageUserDetail(Integer userId){
        if(userId == null){
            return ServerResponse.createByErrorCodeMessage(ResponseCode.ILLEGAL_ARGUMENT.getCode(),ResponseCode.ILLEGAL_ARGUMENT.getDesc());
        }
        User user = userMapper.selectByPrimaryKey(userId);
        if(user == null){
            return ServerResponse.createByErrorMessage("該用戶不存在");
        }
        UserDetailVo userDetailVo = assembleUserDetailVo(user);
     ...
     * @return
     */
    private UserDetailVo assembleUserDetailVo(User user){
        ...
        userDetailVo.setCreateTime(DateTimeUtil.dateToStr(user.getCreateTime()));
        userDetailVo.setUpdateTime(DateTimeUtil.dateToStr(user.getUpdateTime()));
        return userDetailVo;
    }
    ...
    public ServerResponse saveOrUpdateUser(User user){
        if(user != null)
        {
            if(user.getId() != null){  // 更新
                user.setRole(Const.Role.ROLE_CUSTOMER);
                int rowCount = userMapper.updateByPrimaryKey(user);
                if(rowCount > 0){
                    return ServerResponse.createBySuccess("更新用户会员信息成功");
                }
                return ServerResponse.createBySuccess("更新会员信息失败");
            }else{   // 新增
                ServerResponse validResponse = this.checkValid(user.getUsername(),Const.USERNAME);
                if(!validResponse.isSuccess()){
                    return validResponse;
                ...
                int resultCount = userMapper.insert(user);
                if(resultCount == 0){
                    return ServerResponse.createByErrorMessage("新增会员信息失败");
                }
                return ServerResponse.createBySuccess("新增会员信息成功");
            }
        }
        return ServerResponse.createByErrorMessage("新增或更新会员信息参数不正确");
    }
     ...
     * @return
     */
    public ServerResponse<PageInfo> searchUser(String username,Integer userId,int pageNum,int pageSize){
        PageHelper.startPage(pageNum,pageSize);
        ...
        List<User> userList = userMapper.selectByNameAndUserId(username,userId);
        List<UserListVo> userListVoList = Lists.newArrayList();
        for(User userItem : userList){
            UserListVo userListVo = assembleUserListVo(userItem);
            userListVoList.add(userListVo);
     ...
     * @return
     */
    public ServerResponse<String> del(Integer userId){
